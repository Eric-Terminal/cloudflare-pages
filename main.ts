...
class VerifyLandingPage {
  ...
  private isPointerOverCard = false;
  private lastTiltX = 0;
  private lastTiltY = 0;
  ...
  private isPointInsideCard(x: number, y: number): boolean {
    if (!this.tiltBounds) {
      this.tiltBounds = this.card.getBoundingClientRect();
    }
  ...
  private bindFinalPlayOption(): void {
    const finalShell = this.card.querySelector<HTMLElement>("#final-shell");
    const playButton = this.card.querySelector<HTMLButtonElement>("#play-rick-button");
